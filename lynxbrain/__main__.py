...
def main() -> None:
    logging.basicConfig(level=os.getenv("LYNX_LOG_LEVEL", "INFO").upper(), format="%(asctime)s %(levelname)s %(name)s: %(message)s")
    ...
    server = AppServer((os.getenv("LYNX_BIND", "0.0.0.0"), int(os.getenv("LYNX_PORT", "8088"))), engine, os.getenv("LYNX_API_TOKEN", ""))
